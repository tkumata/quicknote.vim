...
Quick Note は、Vim 上で Obsidian のようなノート運用を実現するためのプラグインです。既存の Obsidian Vault ディレクトリ配下のファイルをそのまま利用できるため、Vault フォルダ構造を汚染することなくノート運用が可能です。本プラグインを利用することで、Obsidian と同じ環境で同一のデータ (Vault) を操作できます。

Personal Knowledge Base (PKB) も視野に入れ、PKB 運用に必要なコマンドも実装しています。
...
- `plugin/quicknote.vim` (本体設定ファイル)
  ...
  - リンクの展開: `[[link]]` にカーソルを置き、`<Enter>` キーを押すとリンク先が開きます。存在しない場合は自動でファイルを作成します。
  - template token 置換
- `my.vim` (補足設定ファイル)
...
- `:NoteBrokenLinks`
  ...
  - 初期仕様では主に `[[name]]` 形式 (Wiki リンク形式) のみを対象とします。Markdown のインラインリンクや URL の有効性チェックは範囲外となります。
  - FZF が利用できない場合は error を表示します。
- `:NoteHelp`
...
- Vim のインストール
- `vim-plug` のインストール
- `find` コマンドが使えるように設定されている環境
- `grep` コマンドが使えるように設定されている環境
...
別のディレクトリで運用されている場合は、`quicknote_root` を変更します。
